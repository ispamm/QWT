...
class QWavelet(object):
    ...
    def qwt(self, image, gl, gh, fl, fh, only_low=True, quad=1):
        ...
        if quad=="all":
            ...
            # Mean of the components (just for checking purposes, not used in paper)
            ll = (lflg + lglf + lflf)/3
            lh = (lfhg + lghf + lfhf)/3
            hl = (hflg + hglf + hflf)/3
            hh = (hfhg + hghf + hfhf)/3
...
def repeat_kmeans(joint_pdfs, subbands, ntimes):
    ...
    return Counter(max_energy_subbands), subbands_dict


def visualize_clusters(pdfs, dict):
    values = []
    ...
    label_len = values.shape[0]//4
    labels = [0]*label_len + [1]*label_len + [2]*label_len + [3]*label_len
    values = values.reshape(values.shape)
    X_embedded = TSNE(n_components=2, learning_rate='auto', init='random').fit_transform(values)
...
def main(dataset, kmeans_repetitions, visualize=False):
    qw = QWavelet()

    if dataset == "kvasir":
        path = "./Datasets\Kvasir_im_mask\images"
        ...
        path = "./chaos2019/png8020/train"
        path1 = "./chaos2019/png8020/train/ct"
        path2 = "./chaos2019/png8020/train/t1"
        path3 = "./chaos2019/png8020/train/t2"
        ...
        path = "./chaos2019"
        files = ChaosDataset_Syn_new(path=path, split='train', modals=('t1', 't2', 'ct'), image_size=128)


    images = []
    counter = 0 # Numer of repetitions
    ...
    counter_dictionary, subbands_dict = repeat_kmeans(joint_pdfs=joint_pdfs, subbands=subbands, ntimes=kmeans_repetitions)
    print(counter_dictionary)

    if visualize:
        visualize_clusters(joint_pdfs, subbands_dict)


main("ixi", kmeans_repetitions=1, visualize=True)
...
# CHAOS T2: Counter({10: 20, 0: 20, 11: 14, 4: 9, 13: 8, 12: 7, 7: 2})
